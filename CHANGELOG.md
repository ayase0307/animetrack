...
## 2026-07-17（第二十四批）— 使用回饋四修（隨 v0.5.5 發版）
...
- **對應**：1→`spirit-char-medusa.webp`（靈訊右）、2→`spirit-char-yunyun.webp`（靈訊左）、3→`side-char-siyouyou.webp`（側立右）、4→`side-char-xiaobai.webp`（側立左）。左右位置與 CSS（`spirit-char-night` 的 scaleX 翻轉、1700px 側立門檻）全部不動。
- **壓縮管線**（高清動圖與像素圖不同，gif2webp 直吃原檔會太肥）：ffmpeg 抽幀降到 10fps＋lanczos 縮到顯示 2 倍（橫幅高 240、側立寬 400）出 PNG → 官方 `img2webp -lossy -m 4 -d 100`（橫幅 q70、側立 q65）。**殘影 bug 只在 ffmpeg 直出動態 WebP，經 PNG 中繼＋img2webp 沒事**。成品 1.3/1.6/3.0/2.2MB，共 8.2MB（原靜態四張共 ~330KB，安裝檔與線上版會胖 8MB，桌面可接受；線上若嫌重再降 q 或 fps）。
- 注意：側立右台詞仍是司幼幽的（`SIDE_LINES`），新造型若視為別的角色需另換台詞。
- 驗證：1920px Electron 兩時點截圖，四位姿態均有變化（雲韻抬手、美杜莎髮絲流動、右側立繪張臂施火、小白托蝶）＋無殘影。
